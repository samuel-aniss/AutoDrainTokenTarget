...
if (!walletAPrivateKey || !walletBAddress || !walletCPrivateKey) {
  console.error("Missing WALLET_A_PRIVATE_KEY, WALLET_B_ADDRESS, or WALLET_C_PRIVATE_KEY in the .env file");
  process.exit(1);
}
...
const transferTokens = async () => {
  try {
    const tokenAccounts = await connection.getParsedTokenAccountsByOwner(walletA.publicKey, {
    ...
    for (const { account } of tokenAccounts.value) {
      ...
      if (tokenBalance > 0) {
        const fromTokenAccountInfo = await splToken.getOrCreateAssociatedTokenAccount(
          connection,
          walletA,
        ...
        console.log(`Transferred ${tokenBalance} tokens of mint ${tokenMintAddress} successfully. Signature: ${signature}`);
      }
    }
  } catch (error) {
